...
class Model extends React.Component {
  ...
  TABS = ["predict", "train", "refund", "reward"];
  ...
  constructor(props) {
    ...
    const metaDataLocation =
      currentUrlParams.get("metaDataLocation") || "local";
  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      try {
        contractInfo = await storage.getModel(
          this.state.modelId,
      ...
      this.setState({ contractInfo, foundModelInStorage }, async (_) => {
        await this.setContractInstance();
        if (typeof window !== "undefined" && window.ethereum) {
          window.ethereum.on("accountsChanged", (accounts) => {
            this.setState(
              { accounts, addedData: null, rewardData: null },
              (_) => {
  ...
  setContractInstance = async () => {
    ...
    const restrictModelInfo =
      this.state.metaDataLocation !== "local" &&
      !this.state.foundModelInStorage &&
      !validator.isPermitted(networkType, contractAddress);
    this.setState({
    ...
    new ContractLoader(this.web3)
      .load(contractAddress)
      .then(async (collabTrainer) => {
      ...
      .catch((err) => {
        this.notify(
          `There was an error loading the contract at ${contractAddress}. Try using a different network.`,
          { variant: "error", autoHideDuration: 15 * 1000 }
        );
        console.error(err);
      });
  };
  ...
  async normalize(data) {
    ...
    return this.state.classifier.methods
      .norm(convertedData.map((v) => this.web3.utils.toHex(v)))
      .call()
      .then((norm) => {
        norm = this.web3.utils.toBN(norm);
        const _toFloat = this.web3.utils.toBN(this.state.toFloat);
        return convertedData.map((v) =>
          this.web3.utils.toBN(v).mul(_toFloat).div(norm)
        );
      });
  }
  ...
  async setTransformInputMethod() {
    ...
    if (encoder === normalizeEncoderName(Encoder.None)) {
      ...
      UniversalSentenceEncoder.load().then((use) => {
        this.transformInput = async (query) => {
          ...
          let embedding = tf.tidy((_) => {
            ...
            if (
              this.state.featureIndices !== undefined &&
              this.state.featureIndices.length > 0
            ) {
      ...
      mobilenet
        ...
        .then((model) => {
          this.transformInput = async (imgElement) => {
            ...
            const imgEmbedding = await model.infer(imgElement, {
              embedding: true,
            });
            let embedding = tf.tidy((_) => {
      ...
      throw new Error(
        `Couldn't find encoder for ${this.state.contractInfo.encoder}`
      );
    }
  }
  ...
  async setFeatureIndices() {
    return this.state.classifier.methods
      .getNumFeatureIndices()
      .call()
      .then(parseInt)
      .then((numFeatureIndices) => {
        return Promise.all(
          [...Array(numFeatureIndices).keys()].map((i) => {
            return this.state.classifier.methods
              .featureIndices(i)
              .call()
              .then(parseInt);
  ...
  addDataCost() {
    ...
    return this.state.incentiveMechanism.methods
      .getNextAddDataCost(now)
      .call()
      .then(parseInt);
  }
  ...
  async canAttemptRefund(data, isForTaking) {
    ...
    return this.state.dataHandler.methods
      .hasClaimed(
        dataSample,
        data.classification,
        data.time,
      ...
      .then(async (hasClaimed) => {
        ...
        let claimableAmount = await this.state.dataHandler.methods
          .getClaimableAmount(
            dataSample,
            data.classification,
            data.time,
        ...
        if (data.initialDeposit === 0) {
          ...
          const numClaims = await this.state.dataHandler.methods
            .getNumClaims(
              dataSample,
              data.classification,
              data.time,
        ...
        if (isForTaking) {
          ...
          const amountShouldGet = Math.floor(
            (data.initialDeposit * this.state.numGood) /
              this.state.totalGoodDataCount
          );
  ...
  getDisplayableEncodedData(data) {
    ...
    const divideFloatList = [Encoder.MobileNetV2, Encoder.USE].map(
      normalizeEncoderName
    );
  ...
  async getOriginalData(transactionHash) {
    ...
    return this.storages[this.state.storageType]
      ...
      .catch((err) => {
        console.warn(
          `Could not find the original data for ${transactionHash}.`
        );
        console.warn(err);
      });
  }
  ...
  getClassificationName(val) {
    if (
      typeof val !== "number" ||
      val < 0 ||
      val >= this.state.classifications.length
    ) {
      return val;
    }
    try {
      ...
      .then(async (results) => {
        ...
        while (results.length > 0 && numAdded < this.state.numDataRowsLimit) {
          const infos = await Promise.all(
            results
              .splice(0, this.state.numDataRowsLimit - numAdded)
              .filter(customFilter)
              .map(cb)
          );
  ...
  updateContractInfo() {
    return Promise.all([
          ...
          for (let i = 0; i < numClassifications; ++i) {
            this.state.classifier.methods
              ...
              .then((classificationName) => {
                this.setState({
                  classifications: update(this.state.classifications, {
                    [i]: { $set: classificationName },
                  }),
                });
              });
          }
      ...
      this.state.incentiveMechanism.methods
        .refundWaitTimeS()
        .call()
        .then(parseInt)
  ...
  updateDynamicInfo() {
    return Promise.all([
      this.addDataCost().then((depositCost) => {
  ...
  updateDynamicAccountInfo() {
    return Promise.all([
      Promise.resolve(this.state.accounts && this.state.accounts[0]).then(
        (account) => {
          if (account) {
            return this.state.incentiveMechanism.methods
              .numValidForAddress(account)
              .call()
              .then(parseInt);
          }
        }
      ...
      if (numGood !== undefined) {
        if (totalGoodDataCount > 0) {
          accountScore =
            ((100 * numGood) / totalGoodDataCount).toFixed(2) + "%";
        } else {
          accountScore = "0%";
        }
      } else {
  ...
  updateRefundData() {
    ...
    return this.handleAddedData(
      this.state.refundFromBlock,
      "refund",
      customFilter,
      (d) => {
        // from DataHandler
        ...
        return this.getOriginalData(d.transactionHash)
          .then(async (originalData) => {
            ...
            info.hasEnoughTimePassed = this.hasEnoughTimePassed(
              info,
          ...
          .catch((err) => {
            console.error(
              `Error getting original data for transactionHash: ${d.transactionHash}`
            );
            console.error(err);
          });
      }
  ...
  nextRefundData() {
    ...
    const refundFromBlock =
      this.state.addedData[this.state.addedData.length - 1].blockNumber + 1;
    this.setState(
      {
        refundFromBlock,
        refundPreviousFromBlocks: update(this.state.refundPreviousFromBlocks, {
          $push: [this.state.addedData[0].blockNumber],
        }),
      },
      this.updateRefundData
    );
  ...
  previousRefundData() {
    ...
    this.setState(
      {
        refundFromBlock:
          this.state.refundPreviousFromBlocks[
            this.state.refundPreviousFromBlocks.length - 1
          ],
        ...
        return this.getOriginalData(d.transactionHash)
          .then(async (originalData) => {
            const info = {
              ...
              time,
              blockNumber: d.blockNumber,
              originalData: this.getDisplayableOriginalData(
                originalData,
                isForTaking
              ),
            };
            ...
            return this.canAttemptRefund(info, isForTaking).then(
              (refundInfo) => {
                const {
                  canAttemptRefund = false,
                  claimableAmount = null,
  ...
  nextRewardData() {
    ...
    const rewardFromBlock =
      this.state.rewardData[this.state.rewardData.length - 1].blockNumber + 1;
    this.setState(
      {
        rewardFromBlock,
        rewardPreviousFromBlocks: update(this.state.rewardPreviousFromBlocks, {
          $push: [this.state.rewardData[0].blockNumber],
        }),
      },
      this.updateRewardData
    );
  ...
  previousRewardData() {
    ...
    this.setState(
      {
        rewardFromBlock:
          this.state.rewardPreviousFromBlocks[
            this.state.rewardPreviousFromBlocks.length - 1
          ],
        // Pop the last one off.
        rewardPreviousFromBlocks: update(this.state.rewardPreviousFromBlocks, {
          $splice: [[-1, 1]],
    ...
    this.setState(
      ...
      (_) => {
        ...
        this.transformInput(input)
          .then((input) => {
            this.setState({
              encodedPredictionData: `Encoded data: ${this.getDisplayableEncodedData(
                input
              )}`,
            });
            this.setState({ prediction: "(Predicting)" }, (_) => {
              this.predict(input)
                ...
                .catch((err) => {
                  this.setState({
                    prediction: "(Error predicting. See console for details.)",
                    predicting: false,
                  });
                  console.error(err);
                });
          ...
          .catch((err) => {
            this.setState({
              prediction:
                "(Error transforming input. See console for details.)",
              predicting: false,
            });
  ...
  refund(time) {
    ...
    return this.state.addedData
      ?.filter((d) => d.time === time)
      ?.forEach((d) => {
        return this.state.contractInstance.methods
          .refund(d.data, d.classification, d.time)
  ...
  takeDeposit(time) {
    ...
    return this.state.rewardData
      ?.filter((d) => d.time === time)
      ?.forEach((d) => {
        return this.state.contractInstance.methods
          .report(d.data, d.classification, d.time, d.sender)
          .send({ from: this.state.accounts[0] })
          .on("transactionHash", (_hash) => {
            // TODO Just Update row.
            this.updateRewardData();
  ...
  train() {
    ...
    return this.transformInput(originalData).then((trainData) => {
      ...
      const value =
        this.state.depositCost + (this.state.depositCost > 0 ? 1e14 : 0);
      ...
      // using the main entry CollaborativeTrainer64 to add the data
        ...
        .on("transactionHash", (transactionHash) => {
          sentNotificationKey = this.notify(
            "Data was sent but has not been confirmed yet"
          );
          ...
          if (this.state.storageType !== "none") {
            ...
            return storage
              ...
              .catch((err) => {
                this.notify("Error saving original data to the database.", {
                  variant: "error",
                });
        ...
        .on("error", (err) => {
          console.error(err);
          this.notify(
            'Error adding data. See the console for details. If you were using a private network, it might help to "Reset Account" in your browser or extension (MetaMask).',
            { variant: "error" }
          );
        });
    });
  ...
  renderLoadingContract() {
    return (
      this.state.readyForInput || (
        <div>
          <ClipLoader
            loading={!this.state.readyForInput}
            size="16"
            color="#2196f3"
          />{" "}
          Loading contract and model information.
        </div>
      )
    );
  }
      ...
      <Container>
        <Paper className={this.classes.root} elevation={1}>
          ...
          <div className={this.classes.descriptionDiv}>
            {this.state.checkedContentRestriction && (
              <div>
                {this.state.contractInfo.description && !restrictModelInfo && (
                  <Tooltip
                    placement="top-start"
                    title="The description for this model"
                  >
            ...
            {(restrictContent || restrictModelInfo) && (
              <Typography component="p">
                {
                ...
                <Link href="/about#online-safety" target="_blank">
                  Learn more
                </Link>
                .
              </Typography>
          ...
          <div>
            <Typography component="p">
              This page allows you interact with a model deployed to a
              blockchain. You can hover over (or long press for touch screens)
              certain items to get more details.
            </Typography>
            <Typography component="p">
              ⚠ WARNING When you click/tap on the TRAIN/REFUND/REWARD buttons
              next to data, a transaction will be created for you to approve in
              your browser's tool (e.g. MetaMask). If the transaction is
              approved, you might be sending data to a public dencentralized
              blockchain not controlled by Microsoft. Before approving, you
              should understand the implications of interacting with a public
              blockchain. You can learn more{" "}
          ...
          <div className={this.classes.addToStorageDiv}>
            {this.state.foundModelInStorage === false && (
              <Typography component="p">
                Want to use this model again later? Save a link to it in your
                storage{" "}
                <Link
                  href={`/addDeployedModel?address=${this.state.contractAddress}`}
                >
                  here
                </Link>
                .
          ...
          <div className={this.classes.info}>
            <Tooltip
              placement="top-start"
              title={
                'The number of data samples that you have contributed that were determined to be "good" compared to the total number of "good" samples contributed'
              }
            >
              <Typography component="p">
                <b>Your score: </b>
                {this.state.accountScore !== undefined
                  ? this.state.accountScore
                  : "(loading)"}
                {this.state.totalGoodDataCount !== undefined
                  ? ` (${this.state.numGood || 0}/${
                      this.state.totalGoodDataCount || 0
                    })`
                  : ""}
              </Typography>
            </Tooltip>
              ...
              <Typography component="p">
                ...
                {this.state.refundWaitTimeS !== undefined
                  ? this.state.refundWaitTimeS !== 0
                    ? moment
                        .duration(this.state.refundWaitTimeS, "s")
                        .humanize()
                    : "0 seconds"
                  : "(loading)"}
              </Typography>
              ...
              <Typography component="p">
                ...
                {this.state.anyAddressClaimWaitTimeS !== undefined
                  ? this.state.anyAddressClaimWaitTimeS !== 0
                    ? moment
                        .duration(this.state.anyAddressClaimWaitTimeS, "s")
                        .humanize()
                    : "0 seconds"
                  : "(loading)"}
              </Typography>
          ...
          <div className={this.classes.controls}>
            {renderStorageSelector(
              "Where to store the link between your update and your original unprocessed data",
              this.state.storageType,
              this.handleInputChange,
              this.state.permittedStorageTypes
            )}
          </div>
          <div>
              ...
              <TabContainer>
                ...
                >
                  <div className={this.classes.tabContainer}>
                    ...
                    <Tooltip
                      placement="top-start"
                      title={this.state.encodedPredictionData || ""}
                    >
                      <Typography component="p">
                        <b>
                          Prediction:{" "}
                          {this.state.restrictContent
            ...
            {this.state.tab === this.TRAIN_TAB && (
              <TabContainer>
                <Typography component="p">
                  Improve the model by providing training data and a label for
                  the data.
                </Typography>
                <form
                  id="train-form"
                ...
                >
                  <div className={this.classes.tabContainer}>
                    {this.renderInputBox()}
                    <InputLabel htmlFor="classification-selector">
                      Classification
                    </InputLabel>
                    <Select
                      value={
                        this.state.trainClassIndex <
                    ...
                    >
                      {this.state.classifications.map(
                        (classificationName, classIndex) => {
                          return (
                            <MenuItem
                              key={`class-select-${classIndex}`}
                              value={classIndex}
                            >
                              {this.state.restrictContent
                                ? classIndex
            ...
            {this.state.tab === this.REFUND_TAB && (
              <TabContainer>
                <Typography component="p">
                  You can attempt to get a refund for data that is "good". There
                  are various ways that the system can identify data as "good".
                  Currently it is done by checking if the model's prediction
                  matches the label that you provided when you submitted the
                  data for training. The main idea is that if your label was
                  wrong, then others should submit data to correct the model
                  before you can validate your contribution. Some incorrect data
                  might get submitted but it would be expensive to submit a lot
                  of incorrect data so overall the model should be mostly okay
                  as long as it is being monitored.
                </Typography>
                <div>
                  {this.state.refundPreviousFromBlocks.length > 0 && (
                    <Button
                      ...
                      variant="outlined"
                      color="primary"
                      onClick={this.previousRefundData}
                    >
                      Previous
                    </Button>
                  )}
                  {this.state.hasMoreRefundData && (
                    <Button
                      className={`${this.props.classes.button} ${this.props.classes.nextButtonContainer}`}
                      variant="outlined"
                      color="primary"
                      onClick={this.nextRefundData}
                    >
                      Next
                    </Button>
                  )}
                ...
                {this.state.addedData === null && (
                  <div>
                    <ClipLoader
                      loading={this.state.addedData === null}
                      size="16"
                      color="#2196f3"
                    />{" "}
                    Looking for data added by you.
                  </div>
                )}
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {this.state.addedData.map((d) => {
                        return (
                          <TableRow key={`refund-row-${d.time}`}>
                            <Tooltip
                              title={`Encoded data: ${this.getDisplayableEncodedData(
                                d.data
                              )}`}
                            >
                              <TableCell>
                                {d.originalData}
                                {d.dataMatches === false &&
                                  " ⚠ The actual data doesn't match this!"}
                              </TableCell>
                            </Tooltip>
                            <TableCell>
                              {this.state.restrictContent
                                ? d.classification
                                : this.getClassificationName(d.classification)}
                            </TableCell>
                              ...
                              {d.errorCheckingStatus ? (
                                ...
                                d.canAttemptRefund ? (
                                  <Button
                                    className={this.classes.button}
                                    variant="outlined"
                                    onClick={() => this.refund(d.time)}
                                  >
                                    Refund{" "}
                                    {this.getHumanReadableEth(
                                      d.claimableAmount
                                    )}
                                  </Button>
                                ) : d.alreadyClaimed ? (
                                  ...
                                  `Classification does not match. Got "${
                                    this.state.restrictContent
                                      ? d.prediction
                                      : this.getClassificationName(d.prediction)
                                  }".`
                                ) : (
                                  "Can't happen?"
                                )
            ...
            {this.state.tab === this.REWARD_TAB && (
              <TabContainer>
                <Typography component="p">
                  You can attempt to report data from other accounts that you
                  believe is incorrect.
                </Typography>
                {(this.state.numGood === 0 ||
                  this.state.numGood === undefined) && (
                  <Typography component="p">
                    You must have some data submitted and collected refunds for
                    it before trying to take another's deposits.
                  </Typography>
                )}
                <div>
                  {this.state.rewardPreviousFromBlocks.length > 0 && (
                    <Button
                      ...
                      variant="outlined"
                      color="primary"
                      onClick={this.previousRewardData}
                    >
                      Previous
                    </Button>
                  )}
                ...
                {this.state.rewardData === null && (
                  <div>
                    <ClipLoader
                      loading={this.state.rewardData === null}
                      size="16"
                      color="#2196f3"
                    />{" "}
                    Looking for data added by others.
                  </div>
                )}
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {this.state.rewardData.map((d) => {
                        return (
                          <TableRow key={`reward-row-${d.time}`}>
                            <Tooltip
                              title={`Encoded data: ${this.getDisplayableEncodedData(
                                d.data
                              )}`}
                              ...
                              {d.errorCheckingStatus ? (
                                ...
                                d.canAttemptRefund ? (
                                  <Button
                                    className={this.classes.button}
                                    variant="outlined"
                                    onClick={() => this.takeDeposit(d.time)}
                                  >
                                    Take{" "}
                                    {this.getHumanReadableEth(
                                      d.claimableAmount
                                  ...
                                  `Classification must be wrong for you to claim this. Got "${
                                    this.state.restrictContent
                                      ? d.prediction
                                      : this.getClassificationName(d.prediction)
                                  }".`
  ...
  renderInputBox() {
    ...
    switch (this.state.inputType) {
              ...
              <section>
                <div {...getRootProps()}>
                  ...
                  <img
                    ...
                    alt="The item to classify or train with."
                    src={
      ...
      case INPUT_TYPE_TEXT:
        return (
          <div className={this.classes.form}>
            <TextField
              inputProps={{ "aria-label": "Input to the model" }}
              name="input"
              label="Input"
              onChange={this.handleInputChange}
              margin="normal"
        ...
        return (
          <div className={this.classes.form}>
            <Typography component="p">
              ...
              {normalizeEncoderName(this.state.encoder) ===
                normalizeEncoderName(Encoder.Mult1E9Round) &&
              ...
              The data will be converted to hexadecimal before being given to
              the smart contract.
            </Typography>
            <TextField
              inputProps={{ "aria-label": "Input to the model" }}
      ...
      default:
        console.warn(
          `No input box rendered for inputType: ${this.state.inputType}`
        );
        return <div></div>;
    }
  }
